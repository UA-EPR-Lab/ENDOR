...
class ProcessSpectra:
    ...
    def __init__(self, filename=0, file_dictionary=0, rawdata=0, phased=0, phase_angle=0):
        ...
        def create_dict(self):
            ...
            if self.filename.endswith(".DSC"):
                ...
                try:
                    self.dict_filename = self.filename.replace(".DTA", ".DSC")
        ...
        self.filename = choose_files(self)
    ...
    def get_from_dict(key):
                ...
                value = spectrum.file_dictionary[key]
    ...
    def lineplot(self):
        ...
        xmin = float("".join(ProcessSpectra.get_from_dict('XMIN')))
        xwid = float("".join(ProcessSpectra.get_from_dict('XWID')))
...
class MimsENDOR(ProcessSpectra):
    ...
    def __init__(self):
        ...
        def buildxy(self):
            ...
            xdim = float("".join(ProcessSpectra.get_from_dict('XPTS')))
            # xdim_string = str(xdim)
            # pad = 0
            # xdim_pad = np.pad(xdim, (pad, pad), 'constant')
            # return xdim_pad
            xmin = float("".join(ProcessSpectra.get_from_dict('XMIN')))
            xrange = float("".join(ProcessSpectra.get_from_dict('XWID')))
    ...
    def calc_endorfreq(self):
        ...
        try:
            b0vl = float("".join(ProcessSpectra.get_from_dict('B0VL')))
        except:
            root = tk.Tk()
            root.withdraw()
            try:
    ...
    def plot(self):
        ProcessSpectra.lineplot()

spectrum = MimsENDOR()
figure = MimsENDOR.plot()
